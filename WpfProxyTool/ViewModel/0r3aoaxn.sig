...
namespace WpfProxyTool.ViewModel
{
    ...
    public class MainViewModel : ViewModelBase
    {
        ...
        {
            for (int i = 0; i < leechList.Count; i+=taskCount)
            {
                Task<int>[] tasks = new Task<int>[taskCount];
                for (int t = i; t < i + taskCount; t++)
                {
                    if (t < leechList.Count)
                    {
                        tasks[i] = Task<int>.Factory.StartNew(makeTaskRequest, leechList[t]);
                    }
                }
        ...
        private async void makeTaskRequest(ProxyLeechListModel item)
        {
            WebContent source = new WebContent();
            source = await getURLContentAsync(item.URL);
